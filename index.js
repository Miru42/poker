...
function compareHands(eval1, eval2) {
    ...
    const getEffectiveHighCardForStraight = (handEval) => {
    ...
    if ((p1.isAllIn || p1.isFolded) && (p2.isAllIn || p2.isFolded)) {
        ...
        if (p1.hasActedThisRound && p2.hasActedThisRound) {
            if (currentBettingRound === 'preflop' && playerWhoLastRaised === (dealerPosition === 'human' ? 'ai' : 'human')) {
                if (currentPlayerTurn === p2.seat && p2.lastAction === 'check' && p2.currentBetInRound === dynamicBigBlind)
                    roundOver = true;
                else if (currentPlayerTurn === p1.seat && p1.lastAction === 'check' && p1.currentBetInRound === dynamicBigBlind)
                    roundOver = true;
    ...
    if (humanPlayer && humanPlayer.isFolded) {
        ...
        if (comparison === humanEval && humanEval.rankValue > aiEval.rankValue) {
            ...
            if (trueSplit)
                ...
                winner = comparison === humanEval ? humanPlayer : aiPlayer;
        }
