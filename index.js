...
async function handleAllInAction() {
    ...
    if (humanTotalChipsBeforeAction <= 0)
        return;
    let amountCommittedByHumanToPotThisAction = humanTotalChipsBeforeAction;
    let finalHumanBetInRound = humanPlayer.currentBetInRound + humanTotalChipsBeforeAction;
    ...
    if (level === 1) { // Very basic AI
        if (canCheck) {
            if (handStrength >= 2 && Math.random() < 0.3) { // Pair or better, sometimes bet
                ...
                if (human && !human.isAllIn && betAmount > human.chips) {
                    betAmount = human.chips;
                } // Cap by human stack
                if (betAmount > 0)
        ...
        if (handStrength >= 5 || (hasStrongDraw && Math.random() < 0.5 + bluffFactor)) {
            if (ai.chips > amountToCallForAI + minRaiseVal) {
                ...
                if (ai.chips <= (targetTotalBet - ai.currentBetInRound))
                    return { type: 'allin', amount: ai.currentBetInRound + ai.chips };
                if (targetTotalBet > betToCallVal)
...
function getImpossibleAIAction(ai, human, community, betToCallVal, minRaiseVal, bigBlindVal, potVal) {
    ...
    if (comparisonResult === aiFinalEval && aiFinalEval.rankValue > humanFinalEval.rankValue) {
        ...
        if (outcome === 'split') {
            ...
            if (!trueSplit)
                outcome = comparisonResult === aiFinalEval ? 'ai_wins' : 'human_wins';
        }
    }
    else {
        outcome = 'human_wins';
    ...
    if (outcome === 'ai_wins') {
        ...
        if (canCheck) {
            ...
            betAmount = Math.max(betAmount, bigBlindVal);
            if (!human.isAllIn && betAmount > human.chips) {
                betAmount = human.chips;
            } // Cap by human stack
            if (ai.chips === betAmount)
                return { type: 'allin', amount: ai.currentBetInRound + betAmount };
            ...
            if (targetTotalBet > betToCallVal)
                return { type: 'raise', amount: targetTotalBet };
            if (ai.chips <= amountToCallForAI)
                return { type: 'allin', amount: ai.currentBetInRound + ai.chips }; // Fallback to call if raise invalid
            return { type: 'call' };
        }
    }
    ...
    if (winner === 'split' && humanPlayer && aiPlayer) {
        ...
        if (remainder > 0) {
            if (dealerPosition === 'human' && aiPlayer)
                aiPlayer.chips += remainder;
            else if (dealerPosition === 'ai' && humanPlayer)
                humanPlayer.chips += remainder;
            else if (humanPlayer)
                humanPlayer.chips += remainder;
        }
